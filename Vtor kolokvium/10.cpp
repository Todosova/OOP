...
Оператор += за додавање нов оглас во низата од огласи. (5 поени) Ако цената на огласот што се внесува е негативна, треба да се
генерира исклучок NegativnaVrednost (објект од класата NegativnaVrednost што посебно треба да се дефинира).
...
Функција void oglasOdKategorija(const char *k) со која се печатат сите огласи од категоријата k што е
проследена како влезен аргумент на методот. (5 поени)
...
class Oglasnik
{
    ...
    Oglasnik()
    {
        strcpy(this->naziv,"");
        this->n=0;
        this->oglas=new Oglas[0];
    }
 /*   Oglasnik( char *naziv,Oglas *oglas,int n)
    {
        strcpy(this->naziv,naziv);
        this->n=n;
        this->oglas=new Oglas[n];
        for(int i =0; i<n;i++)
        {
            this->oglas[i]=oglas[i];
        }
    }*/
     Oglasnik( char *naziv)
    {
    ...
    Oglasnik(const Oglasnik &ok)
    {
        strcpy(this->naziv,ok.naziv);
        this->n=ok.n;
        this->oglas=new Oglas[ok.n];
        for(int i =0; i<ok.n;i++)
        {
            this->oglas[i]=ok.oglas[i];
        }
    }
     Oglasnik &operator=(const Oglasnik &ok)
    {
        if(this!=&ok)
        {
        strcpy(this->naziv,ok.naziv);
    ...
    friend ostream &operator<<(ostream &out,Oglasnik &ok)
    {

        out<<ok.naziv<<endl;
        for(int i=0;i<ok.n;i++)
        {
          out<<ok.oglas[i].getNaslov()<<endl;
        out<<ok.oglas[i].getOpis()<<endl;
        out<<ok.oglas[i].getCena()<<" evra"<<endl;
