...
private:
    int casovi_nedela;
public:
    Demonstrator(int indeks, int *ocenki, int brojOcenki, char *imeIPrezime, Kurs *kursevi, int brojKursevi,int casoviNedela) : Student(indeks,ocenki,brojOcenki),
                                                                                                                                Predavach(imeIPrezime,kursevi,brojKursevi) {
        this->casovi_nedela=casoviNedela;
    }
    ...
    int getBodovi(){
        ...
        try {
            if (getBrojKursevi()>0)
                return bodovi + ((20 * casovi_nedela) / getBrojKursevi());
            else throw NoCourseException(indeks);
        }
...
Student& vratiNajdobroRangiran(Student ** studenti, int n ){
    int max= studenti[0]->getBodovi(),ind=0;
    for (int i = 0; i < n; ++i) {
        try {
...
void pecatiDemonstratoriKurs (char* kurs, Student** studenti, int n){
    for (int i = 0; i < n; ++i) {
        if (dynamic_cast<Demonstrator*>(studenti[i])){
            for (int j = 0; j < studenti[i]->getBrojOcenki(); ++j) {
                if (strcmp(kurs,dynamic_cast<Demonstrator*>(studenti[i])->getKursevi()[j].getIme())==0){
                    studenti[i]->pecati();
                    cout<<"\n";
                    break;
                }
